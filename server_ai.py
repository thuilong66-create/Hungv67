"""
🏆 SERVER AI - VIP 4-IN-1 v16.1 (Sửa lỗi nối đuôi) (Auto+Random+ML+AI_PRO) (Auto+Random+ML)
═══════════════════════════════════════════════
...
app = FastAPI(title="🏆 VIP 4-IN-1 v16.1 (Sửa lỗi nối đuôi) (Auto+Random+ML+AI_PRO) (Auto+Random+ML)", version="15.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def thuat_toan_theo_cau_60(kq_list):
    ...
    logger.info(f"🗳️ Bầu cử 60 dạng: TÀI={phieu_tai} XỈU={phieu_xiu} | {du_doan} thắng | {ti_le}% | {tong_phieu_hoat_dong}/50 tín hiệu")
...
def luu_va_hoc_markov(tool, kq_list):
    """Lưu chuỗi lịch sử vào DB để Markov học"""
    if len(kq_list) < 4:
        return
    try:
        with get_db() as conn:
            c = conn.cursor()
            # Lưu cặp (3 phiên cuối → phiên kế tiếp)
            for i in range(len(kq_list) - 3):
                chuoi = "".join(["T" if x == "Tài" else "X" for x in kq_list[i:i+3]])
                ket_qua = "T" if kq_list[i+3] == "Tài" else "X"
                c.execute("INSERT INTO lich_su_markov (tool, chuoi, ket_qua) VALUES (?, ?, ?)",
                         (tool, chuoi, ket_qua))
            conn.commit()
    except Exception as e:
        logger.warning(f"⚠️ Lỗi lưu Markov: {e}")

def du_doan_markov(tool, kq_list):
    """🧠 AI Học máy Markov Chain: dựa trên 3 phiên cuối, tìm trong lịch sử xem tiếp theo thường là gì"""
    if len(kq_list) < 3:
        return None
    chuoi_hien_tai = "".join(["T" if x == "Tài" else "X" for x in kq_list[-3:]])
    try:
        with get_db() as conn:
            c = conn.cursor()
            c.execute("""
                SELECT ket_qua, COUNT(*) as cnt 
                FROM lich_su_markov 
                WHERE tool = ? AND chuoi = ? 
                GROUP BY ket_qua 
                ORDER BY cnt DESC
            """, (tool, chuoi_hien_tai))
            rows = c.fetchall()
            if not rows:
                # Thử với 2 phiên cuối
                chuoi2 = "".join(["T" if x == "Tài" else "X" for x in kq_list[-2:]])
                c.execute("""
                    SELECT ket_qua, COUNT(*) as cnt 
                    FROM lich_su_markov 
                    WHERE tool = ? AND chuoi LIKE ? 
                    GROUP BY ket_qua 
                    ORDER BY cnt DESC
                """, (tool, "%" + chuoi2))
                rows = c.fetchall()
            if rows:
                ...
                top = rows[0]
                xac_suat = round(top[1] / tong * 100, 1)
                ket_qua = "TÀI" if top[0] == "T" else "XỈU"
                ti_le = max(55.0, min(55 + (xac_suat - 50) * 0.8, 90.0))
                logger.info(f"🧠 Markov: chuỗi {chuoi_hien_tai} → {ket_qua} ({xac_suat}% từ {tong} mẫu)")
                return {"du_doan": ket_qua, "ti_le": ti_le}
    except Exception as e:
        logger.warning(f"⚠️ Lỗi Markov: {e}")
    return None
...
async def home():
    ...
    return {"status": "ok", "msg": "🏆 VIP 4-IN-1 v16.1 (Sửa lỗi nối đuôi) (Auto+Random+ML+AI_PRO) (Auto+Random+ML) đang chạy"}
...
if __name__ == "__main__":
    ...
    logger.info(f"🏆 VIP 4-IN-1 v16.1 (Sửa lỗi nối đuôi) (Auto+Random+ML+AI_PRO) (Auto+Random+ML) | Bỏ phiếu đa số | Cổng {port}")
