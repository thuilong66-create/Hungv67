...
Thuật toán: 8 loại cầu phổ biến Tài Xỉu
...
# ================= THUẬT TOÁN SOI CẦU NÂNG CẤP — 8 LOẠI CẦU =================
def phan_tich_ai(kq_list):
    ...
    if tong_phien < 5:
        return {
            "du_doan": "WAIT",
            "ti_le": 0,
            "loi_khuyen": "Đang thu thập dữ liệu...",
            "tong_tai": tong_tai,
            "tong_xiu": tong_xiu,
            "phuong_phap": "Chờ ≥ 5 phiên",
            "chuoi_tai": 0,
            "chuoi_xiu": 0
        }

    kq_cuoi = kq_list[-1]
    kq2 = kq_list[-2] if tong_phien >= 2 else None
    kq3 = kq_list[-3] if tong_phien >= 3 else None
    kq4 = kq_list[-4] if tong_phien >= 4 else None
    kq5 = kq_list[-5] if tong_phien >= 5 else None
    ...
    # ========== 1. CẦU BỆT DÀI (Liên tiếp 4+ cùng loại) ==========
    if chuoi_tai >= 4:
        du_doan = "XỈU"
        ty_le = min(90 + chuoi_tai * 2, 98.5)
        pp = f"Bệt dài TÀI {chuoi_tai} → bẻ XỈU"
    
    elif chuoi_xiu >= 4:
        du_doan = "TÀI"
        ty_le = min(90 + chuoi_xiu * 2, 98.5)
        pp = f"Bệt dài XỈU {chuoi_xiu} → bẻ TÀI"

    # ========== 2. CẦU XEN KẼ T-X-T-X ==========
    elif tong_phien >= 5:
        mau_5 = kq_list[-5:]
        if mau_5 == ["Tài", "Xỉu", "Tài", "Xỉu", "Tài"]:
            du_doan = "XỈU"
            ty_le = 92.5
            pp = "Xen kẽ T-X-T-X-T → XỈU"
        
        elif mau_5 == ["Xỉu", "Tài", "Xỉu", "Tài", "Xỉu"]:
            du_doan = "TÀI"
            ty_le = 92.5
            pp = "Xen kẽ X-T-X-T-X → TÀI"

        # ========== 3. CẦU ĐÔI SONG TỬ ==========
        elif tong_phien >= 4 and kq5 == kq3 and kq2 == kq4 and kq5 != kq2:
            du_doan = kq5
            ty_le = 88.0
            pp = "Đôi song tử → lặp lại"

        # ========== 4. CẦU CẠNH BẰNG (TT-XX / XX-TT) ==========
        elif tong_phien >= 4:
            bo_4 = kq_list[-4:]
            if bo_4 == ["Tài", "Tài", "Xỉu", "Xỉu"]:
                du_doan = "TÀI"
                ty_le = 85.5
                pp = "Cạnh bằng TT-XX → TÀI"
            
            elif bo_4 == ["Xỉu", "Xỉu", "Tài", "Tài"]:
                du_doan = "XỈU"
                ty_le = 85.5
                pp = "Cạnh bằng XX-TT → XỈU"

        # ========== 5. CẦU LÁI NGƯỢC 3 ==========
        elif kq3 == kq2 and kq2 != kq_cuoi:
            du_doan = kq2
            ty_le = 82.0
            pp = f"Lái ngược {kq3}-{kq2}-{kq_cuoi} → {kq2}"

        # ========== 6. CẦU CHÊNH LỆCH LỚN (≥10 phiên) ==========
        elif tong_phien >= 10:
            ti_le_tai = tong_tai / tong_phien
            if ti_le_tai >= 0.65:
                du_doan = "XỈU"
                ty_le = 78.0
                pp = f"Chênh lệch TÀI {tong_tai}/{tong_phien} → cân XỈU"
            
            elif ti_le_tai <= 0.35:
                du_doan = "TÀI"
                ty_le = 78.0
                pp = f"Chênh lệch XỈU chiếm đa số → cân TÀI"
...
async def scan_game(tool: str, username: str):
    ...
    try:
        ...
        logger.info(f"✅ {ket_qua['du_doan']} ({ket_qua['ti_le']}%) | {ket_qua['phuong_phap']}")
