"""
🏆 SERVER AI - VIP ULTIMATE v10.0
═══════════════════════════════════════════════
✨ THUẬT TOÁN VIP MẠNH NHẤT - TỔNG HỢP ULTIMATE
   🧠 15+ dạng cầu chuyên nghiệp
   ⚖️ Logic THEO/BẺ thích ứng động (tự điều chỉnh theo cầu)
   📊 Phân tích đa khung đồng thuận (3-5-8-15 phiên)
   🎯 Trọng số động: tín hiệu phù hợp với hiện trạng = trọng số cao
   🔍 Phát hiện cầu vỡ sớm (dấu hiệu đảo chiều)
...
app = FastAPI(title="🏆 VIP ULTIMATE v10.0", version="10.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# ═══════════════════════════════════════════════════════════
# 🏆 VIP ULTIMATE - HỆ THỐNG TÍN HIỆU + TRỌNG SỐ ĐỘNG
# ═══════════════════════════════════════════════════════════

class TinHieu:
    def __init__(self, ket_qua, diem_goc, ten_cau, hanh_dong, mo_ta=""):
        self.ket_qua = ket_qua
        self.diem_goc = diem_goc      # Điểm gốc 0-100
        self.ten_cau = ten_cau
        self.hanh_dong = hanh_dong    # "THEO" hoặc "BẺ"
        self.mo_ta = mo_ta
        self.trong_so = 1.0           # Trọng số động (sẽ được tính sau)
...
def phan_tich_da_khung(kq):
    """Phân tích đa khung thời gian → trả về xu hướng từng khung"""
    ket_qua = {}
...
def thuat_toan_vip_ultimate(kq_list):
    """
    🏆 VIP ULTIMATE - Thuật toán tổng hợp mạnh nhất
    ------------------------------------------------
    Bước 1: Phân tích hiện trạng cầu (đa khung, chuỗi hiện tại)
    Bước 2: Thu thập 15+ tín hiệu từ các dạng cầu
    Bước 3: Tính trọng số động cho mỗi tín hiệu (phù hợp với hiện trạng = cao)
    Bước 4: Tổng hợp điểm có trọng số → Quyết định cuối + Điểm tin cậy
    """
    ...
    so_kung_dong_tinh_tai = sum(1 for v in da_khung.values() if v == "TÀI")
    so_kung_dong_tinh_xiu = sum(1 for v in da_khung.values() if v == "XỈU")
    
    signals = []
    
    # ═══════════════════════════════════════════
    # 🏗️ BƯỚC 1: THU THẬP TÍN HIỆU (15+ dạng)
    # ═══════════════════════════════════════════
    
    # 1. CẦU BỆT - Logic thích ứng THEO/BẺ
    chuoi_hien_tai = max(chuoi_tai, chuoi_xiu)
    if chuoi_hien_tai >= 2:
        loai_hien_tai = "Tài" if chuoi_tai >= chuoi_xiu else "Xỉu"
        doi_phuong = "XỈU" if loai_hien_tai == "Tài" else "TÀI"
        
        if chuoi_hien_tai == 2:
            signals.append(TinHieu("TÀI" if loai_hien_tai == "Tài" else "XỈU", 74, "Cầu bệt 2p", "THEO", f"Bệt 2p {loai_hien_tai} → THEO"))
        elif chuoi_hien_tai == 3:
            signals.append(TinHieu("TÀI" if loai_hien_tai == "Tài" else "XỈU", 82, "Cầu bệt 3p", "THEO", f"Bệt 3p {loai_hien_tai} → THEO"))
        elif chuoi_hien_tai == 4:
            # 4 phiên: THEO nhưng yếu, có dấu hiệu già
            signals.append(TinHieu("TÀI" if loai_hien_tai == "Tài" else "XỈU", 72, "Cầu bệt 4p", "THEO", f"Bệt 4p {loai_hien_tai} → THEO (yếu)"))
            # Đồng thời tín hiệu BẺ yếu (cầu bắt đầu già)
            signals.append(TinHieu(doi_phuong, 58, "Cầu bệt già", "BẺ", f"Bệt 4p → cảnh báo BẺ"))
        elif chuoi_hien_tai == 5:
            # 5 phiên: ngưỡng quyết định - BẺ mạnh hơn THEO
            signals.append(TinHieu(doi_phuong, 80, "Cầu bệt 5p", "BẺ", f"Bệt 5p {loai_hien_tai} → BẺ {doi_phuong}"))
            signals.append(TinHieu("TÀI" if loai_hien_tai == "Tài" else "XỈU", 60, "Cầu bệt 5p", "THEO", f"Bệt 5p → THEO (yếu)"))
        elif chuoi_hien_tai >= 6:
            # ≥6 phiên: BẺ mạnh, cầu sắp vỡ
            diem_be = min(82 + (chuoi_hien_tai - 6) * 2, 92)
            signals.append(TinHieu(doi_phuong, diem_be, f"Cầu bệt dài {chuoi_hien_tai}p", "BẺ", f"Bệt {chuoi_hien_tai}p → BẺ {doi_phuong}"))
        ...
        if all(m4[i] != m4[i+1] for i in range(3)):
            ...
            if tong >= 6:
                ...
                if all(m6[i] != m6[i+1] for i in range(5)):
                    signals.append(TinHieu(du_doan, 93, "Cầu nhịp 1-1 HH", "THEO", "T-X 6p hoàn hảo → THEO"))
                    if tong >= 8:
                        m8 = kq_list[-8:]
                        if all(m8[i] != m8[i+1] for i in range(7)):
                            signals.append(TinHieu(du_doan, 95, "Cầu nhịp 1-1 SIÊU", "THEO", "T-X 8p siêu hoàn hảo → THEO"))
                else:
                    signals.append(TinHieu(du_doan, 84, "Cầu nhịp 1-1", "THEO", "T-X 4p → THEO"))
            else:
                signals.append(TinHieu(du_doan, 84, "Cầu nhịp 1-1", "THEO", "T-X 4p → THEO"))
    ...
    if tong >= 6:
        ...
        if m6 == ["Tài", "Tài", "Xỉu", "Tài", "Tài", "Xỉu"]:
            signals.append(TinHieu("TÀI", 89, "Cầu chu kỳ 2-1 HH", "THEO", "TT-X 6p hoàn hảo → THEO TÀI"))
        elif m6 == ["Xỉu", "Xỉu", "Tài", "Xỉu", "Xỉu", "Tài"]:
            signals.append(TinHieu("XỈU", 89, "Cầu chu kỳ 2-1 HH", "THEO", "XX-T 6p hoàn hảo → THEO XỈU"))
        elif tong >= 3:
            m3 = kq_list[-3:]
            if m3 == ["Tài", "Tài", "Xỉu"]:
                signals.append(TinHieu("TÀI", 71, "Cầu chu kỳ 2-1", "THEO", "Dấu hiệu TT-X → THEO TÀI"))
            elif m3 == ["Xỉu", "Xỉu", "Tài"]:
                signals.append(TinHieu("XỈU", 71, "Cầu chu kỳ 2-1", "THEO", "Dấu hiệu XX-T → THEO XỈU"))
    
    # 4. CẦU CÂN BẰNG 2-2 (TT-XX-TT-XX)
    if tong >= 8:
        m8 = kq_list[-8:]
        if m8 == ["Tài", "Tài", "Xỉu", "Xỉu", "Tài", "Tài", "Xỉu", "Xỉu"]:
            signals.append(TinHieu("TÀI", 87, "Cầu cân bằng 2-2 HH", "THEO", "TT-XX 8p → THEO TÀI"))
        elif m8 == ["Xỉu", "Xỉu", "Tài", "Tài", "Xỉu", "Xỉu", "Tài", "Tài"]:
            signals.append(TinHieu("XỈU", 87, "Cầu cân bằng 2-2 HH", "THEO", "XX-TT 8p → THEO XỈU"))
        elif tong >= 4:
            m4 = kq_list[-4:]
            if m4 == ["Tài", "Tài", "Xỉu", "Xỉu"]:
                signals.append(TinHieu("TÀI", 73, "Cầu cân bằng 2-2", "THEO", "Dấu hiệu TT-XX → THEO TÀI"))
            elif m4 == ["Xỉu", "Xỉu", "Tài", "Tài"]:
                signals.append(TinHieu("XỈU", 73, "Cầu cân bằng 2-2", "THEO", "Dấu hiệu XX-TT → THEO XỈU"))
    
    # 5. CẦU CHU KỲ 3-1 (TTT-X-TTT-X)
    if tong >= 8:
        m8 = kq_list[-8:]
        if m8 == ["Tài", "Tài", "Tài", "Xỉu", "Tài", "Tài", "Tài", "Xỉu"]:
            signals.append(TinHieu("TÀI", 86, "Cầu chu kỳ 3-1", "THEO", "TTT-X hoàn hảo → THEO TÀI"))
        elif m8 == ["Xỉu", "Xỉu", "Xỉu", "Tài", "Xỉu", "Xỉu", "Xỉu", "Tài"]:
            signals.append(TinHieu("XỈU", 86, "Cầu chu kỳ 3-1", "THEO", "XXX-T hoàn hảo → THEO XỈU"))
    ...
    if tong >= 6:
        ...
        if k1 == k3 == k5 and k2 == k4 == k6 and k1 != k2:
            ...
            signals.append(TinHieu(du_doan, 85, "Cầu song tử", "THEO", f"Mẫu {k1}-{k2} lặp → THEO"))
    
    # 7. CẦU ĐUÔI GOM (3-4p cuối cùng chiều)
    if tong >= 3:
        m3 = kq_list[-3:]
        if m3.count("Tài") == 3:
            signals.append(TinHieu("TÀI", 79, "Cầu đuôi gom 3p TÀI", "THEO", "3p cuối TÀI → THEO"))
        elif m3.count("Xỉu") == 3:
            signals.append(TinHieu("XỈU", 79, "Cầu đuôi gom 3p XỈU", "THEO", "3p cuối XỈU → THEO"))
        elif tong >= 4:
            m4 = kq_list[-4:]
            if m4.count("Tài") >= 3 and m4[-1] == "Tài":
                signals.append(TinHieu("TÀI", 73, "Cầu đuôi gom 4p TÀI", "THEO", "3/4p cuối TÀI → THEO"))
            elif m4.count("Xỉu") >= 3 and m4[-1] == "Xỉu":
                signals.append(TinHieu("XỈU", 73, "Cầu đuôi gom 4p XỈU", "THEO", "3/4p cuối XỈU → THEO"))
    
    # 8. CẦU THIÊN VỊ (một bên chiếm ưu thế)
    if tong >= 10:
        if tl_tai >= 0.70:
            diem = 80 + min(int((tl_tai - 0.70) * 100), 10)
            signals.append(TinHieu("TÀI", diem, "Cầu thiên TÀI mạnh", "THEO", f"TÀI {round(tl_tai*100)}% → THEO"))
        elif tl_tai >= 0.60:
            signals.append(TinHieu("TÀI", 72, "Cầu thiên TÀI", "THEO", f"TÀI {round(tl_tai*100)}% → THEO"))
        elif tl_tai <= 0.30:
            diem = 80 + min(int((0.70 - tl_tai) * 100), 10)
            signals.append(TinHieu("XỈU", diem, "Cầu thiên XỈU mạnh", "THEO", f"XỈU {round((1-tl_tai)*100)}% → THEO"))
        elif tl_tai <= 0.40:
            signals.append(TinHieu("XỈU", 72, "Cầu thiên XỈU", "THEO", f"XỈU {round((1-tl_tai)*100)}% → THEO"))
    
    # 9. CẦU HỒI QUY (chênh lệch quá lớn → BẺ)
    if tong >= 12:
        if chenh >= 7:
            if tong_tai > tong_xiu:
                signals.append(TinHieu("XỈU", 78, "Cầu hồi quy mạnh", "BẺ", f"Chênh {chenh}p → BẺ XỈU"))
            else:
                signals.append(TinHieu("TÀI", 78, "Cầu hồi quy mạnh", "BẺ", f"Chênh {chenh}p → BẺ TÀI"))
        elif chenh >= 5:
            if tong_tai > tong_xiu:
                signals.append(TinHieu("XỈU", 68, "Cầu hồi quy", "BẺ", f"Chênh {chenh}p → BẺ XỈU"))
            else:
                signals.append(TinHieu("TÀI", 68, "Cầu hồi quy", "BẺ", f"Chênh {chenh}p → BẺ TÀI"))
    
    # 10. ĐẢO CHIỀU NHANH (vừa đảo 1p → BẺ trở lại)
    if tong >= 4:
        k1, k2, k3, k4 = kq_list[-4], kq_list[-3], kq_list[-2], kq_list[-1]
        if k2 == k3 == k4 and k1 != k2:
            if k2 == "Tài":
                signals.append(TinHieu("XỈU", 67, "Cầu đảo chiều", "BẺ", "Vừa đảo TÀI → BẺ XỈU"))
            else:
                signals.append(TinHieu("TÀI", 67, "Cầu đảo chiều", "BẺ", "Vừa đảo XỈU → BẺ TÀI"))
    
    # 11. CẦU GÃY ĐÔI (XX-TT-XX-TT đứt gãy)
    if tong >= 5:
        m5 = kq_list[-5:]
        # XX-TT-X (gãy cặp đôi) → có thể tiếp tục X
        if m5[:2] == ["Xỉu", "Xỉu"] and m5[2:4] == ["Tài", "Tài"] and m5[4] == "Xỉu":
            signals.append(TinHieu("XỈU", 70, "Cầu gãy đôi", "THEO", "XX-TT-X → THEO XỈU"))
        if m5[:2] == ["Tài", "Tài"] and m5[2:4] == ["Xỉu", "Xỉu"] and m5[4] == "Tài":
            signals.append(TinHieu("TÀI", 70, "Cầu gãy đôi", "THEO", "TT-XX-T → THEO TÀI"))
    
    # 12. CẦU 3-2 (TTT-XX-TTT-XX)
    if tong >= 10:
        m10 = kq_list[-10:]
        if m10 == ["Tài"]*3 + ["Xỉu"]*2 + ["Tài"]*3 + ["Xỉu"]*2:
            signals.append(TinHieu("TÀI", 84, "Cầu chu kỳ 3-2", "THEO", "TTT-XX hoàn hảo → THEO TÀI"))
        if m10 == ["Xỉu"]*3 + ["Tài"]*2 + ["Xỉu"]*3 + ["Tài"]*2:
            signals.append(TinHieu("XỈU", 84, "Cầu chu kỳ 3-2", "THEO", "XXX-TT hoàn hảo → THEO XỈU"))
    
    # 13. ĐA KHUNG ĐỒNG THUẬN (3-5-8-15p cùng chiều)
    if so_kung_dong_tinh_tai >= 3:
        signals.append(TinHieu("TÀI", 76, "Đa khung đồng thuận TÀI", "THEO", f"{so_kung_dong_tinh_tai}/4 khung thiên TÀI"))
    elif so_kung_dong_tinh_xiu >= 3:
        signals.append(TinHieu("XỈU", 76, "Đa khung đồng thuận XỈU", "THEO", f"{so_kung_dong_tinh_xiu}/4 khung thiên XỈU"))
    
    # 14. CẦU VỠ SỚM (dấu hiệu cầu bệt sắp vỡ)
    if chuoi_hien_tai >= 3 and tong >= 5:
        # Kiểm tra phiên trước chuỗi bệt có phải ngược chiều không (dấu hiệu vỡ)
        vi_tri_ket_thuc_chuoi = tong - chuoi_hien_tai - 1
        if vi_tri_ket_thuc_chuoi >= 0:
            phien_truoc_chuoi = kq_list[vi_tri_ket_thuc_chuoi]
            loai_chuoi = "Tài" if chuoi_tai >= chuoi_xiu else "Xỉu"
            if phien_truoc_chuoi != loai_chuoi:
                # Chuỗi bệt bắt đầu sau khi đảo → có thể đảo lại
                doi_phuong = "XỈU" if loai_chuoi == "Tài" else "TÀI"
                diem_vo = 55 + chuoi_hien_tai * 3
                signals.append(TinHieu(doi_phuong, min(diem_vo, 70), "Cầu vỡ sớm", "BẺ", f"Dấu hiệu vỡ cầu {chuoi_hien_tai}p → BẺ {doi_phuong}"))
    
    # 15. NỐI ĐUÔI MẶC ĐỊNH (luôn có, điểm thấp)
    if tong >= 3:
        m3 = kq_list[-3:]
    ...
    # ═══════════════════════════════════════════
    # ⚖️ BƯỚC 2: TÍNH TRỌNG SỐ ĐỘNG CHO MỖI TÍN HIỆU
    # ═══════════════════════════════════════════
    # Nguyên tắc: Tín hiệu phù hợp với hiện trạng cầu = trọng số cao hơn
    # - Nếu đang có cầu bệt dài → tín hiệu BẺ có trọng số cao
    # - Nếu đang có cầu nhịp ổn định → tín hiệu THEO nhịp có trọng số cao
    # - Nếu đa khung đồng thuận → tín hiệu cùng chiều có trọng số cao
    
    for s in signals:
        trong_so = 1.0
        
        # Tăng trọng số nếu tín hiệu đồng thuận với đa khung
        if so_kung_dong_tinh_tai >= 3 and s.ket_qua == "TÀI":
            trong_so *= 1.15
        if so_kung_dong_tinh_xiu >= 3 and s.ket_qua == "XỈU":
            trong_so *= 1.15
        
        # Tăng trọng số nếu tín hiệu THEO và đang có chuỗi bệt ngắn
        if s.hanh_dong == "THEO" and 2 <= chuoi_hien_tai <= 3 and s.ket_qua == ("TÀI" if chuoi_tai >= chuoi_xiu else "XỈU"):
            trong_so *= 1.1
        
        # Tăng trọng số nếu tín hiệu BẺ và đang có chuỗi bệt dài
        if s.hanh_dong == "BẺ" and chuoi_hien_tai >= 5:
            trong_so *= 1.2
        
        # Giảm trọng số nếu tín hiệu ngược với đa khung đồng thuận
        if so_kung_dong_tinh_tai >= 3 and s.ket_qua == "XỈU":
            trong_so *= 0.85
        if so_kung_dong_tinh_xiu >= 3 and s.ket_qua == "TÀI":
            trong_so *= 0.85
        
        s.trong_so = round(trong_so, 2)
    
    # ═══════════════════════════════════════════
    # 🏆 BƯỚC 3: TỔNG HỢP ĐIỂM CÓ TRỌNG SỐ
    # ═══════════════════════════════════════════
    nhom_tai = [s for s in signals if s.ket_qua == "TÀI"]
    nhom_xiu = [s for s in signals if s.ket_qua == "XỈU"]
    
    def tinh_diem_ultimate(nhom):
        if not nhom:
            return 0.0
        # Sắp xếp theo điểm × trọng số giảm dần
        sorted_s = sorted(nhom, key=lambda x: -(x.diem_goc * x.trong_so))
        # Tín hiệu mạnh nhất = 100%
        tong_diem = sorted_s[0].diem_goc * sorted_s[0].trong_so
        # Tín hiệu phụ: trọng số giảm dần
        for i, s in enumerate(sorted_s[1:], 1):
            he_so_giam = max(0.35 - (i - 1) * 0.07, 0.08)
            tong_diem += s.diem_goc * s.trong_so * he_so_giam
        return min(round(tong_diem, 1), 95.0)
    
    diem_tai = tinh_diem_ultimate(nhom_tai)
    diem_xiu = tinh_diem_ultimate(nhom_xiu)
    ...
    log_s = " | ".join([
        ...
        for s in sorted(signals, key=lambda x: -(x.diem_goc * x.trong_so))[:5]
    ])
    logger.info(f"🎯 ULTIMATE: {du_doan} | {ti_le}% | {tin_hieu_chinh.ten_cau if tin_hieu_chinh else 'Tổng hợp'} | [{log_s}]")
...
async def home():
    ...
    return {"status": "ok", "msg": "🏆 VIP ULTIMATE v10.0 đang chạy"}
...
async def health():
    return {
        "status": "online",
        "version": "10.0",
        "mode": "🏆 VIP ULTIMATE - Thuật toán mạnh nhất",
        "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    }

@app.get("/api/scan")
async def scan_game(tool: str, username: str):
    logger.info(f"🏆 ULTIMATE Scan: {tool} | {username}")
    ...
    try:
        ...
        # 🏆 CHẠY THUẬT TOÁN VIP ULTIMATE
        ket_qua = thuat_toan_vip_ultimate(lich_su_kq)
        ...
        logger.info(f"🎯 KẾT QUẢ ULTIMATE: Phiên #{phien_hien_thi} → {response['du_doan']} | {response['ti_le']}%")
...
if __name__ == "__main__":
    ...
    logger.info(f"🏆 VIP ULTIMATE v10.0 | Thuật toán mạnh nhất | Cổng {port}")
