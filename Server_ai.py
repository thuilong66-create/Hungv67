...
def phan_tich_ai(kq_list):
    ...
    if len(kq_list) < 5:
        return {"du_doan": "WAIT", "ti_le": 0, "loi_khuyen": "Đang nạp Data...", "trend": "...", "tong_tai": tong_tai, "tong_xiu": tong_xiu}
    kq_cuoi = kq_list[-1]
    chuoi = 1
...
async def scan_game(tool: str, username: str):
    ...
    try:
        ...
        data = phan_tich_ai(kq)
        data["phien"] = str(int(lst[-1]["id"]) + 1)
        return {"status": "success", "data": data}
    except:
        return {"status": "error", "msg": "Bảo trì Server!"}
